...
fn select_manifest_files(project_root: &Path, cfg: &KubeLinterConfig) -> Vec<PathBuf> {
    let roots: Vec<PathBuf> = if cfg.paths.is_empty() {
        ...
        cfg.paths
            .iter()
            .filter_map(|path| resolve_scoped_path(project_root, path))
            .collect()
    };
...
fn resolve_scoped_path(root: &Path, path: &str) -> Option<PathBuf> {
...
fn find_config_file(config_dir: &Path, cfg: &KubeLinterConfig) -> Option<PathBuf> {
    if let Some(config) = cfg.config.as_deref()
        && let Some(configured) = resolve_scoped_path(config_dir, config)
        && configured.is_file()
    {
...
mod tests {
    ...
    #[test]
    fn paths_cannot_escape_their_configured_roots() {
        let project = tempfile::tempdir().unwrap();
        let outside = tempfile::tempdir().unwrap();
        write(
            &outside.path().join("deployment.yaml"),
            "apiVersion: apps/v1\nkind: Deployment\n",
        );
        write(&outside.path().join("kube-linter.yaml"), "checks: {}\n");

        for path in [
            "",
            "../deployment.yaml",
            outside.path().join("deployment.yaml").to_str().unwrap(),
        ] {
            let cfg = KubeLinterConfig {
                paths: vec![path.into()],
                config: None,
        ...
        for config in [
            "",
            "../kube-linter.yaml",
            outside.path().join("kube-linter.yaml").to_str().unwrap(),
        ] {
            let cfg = KubeLinterConfig {
                paths: Vec::new(),
                config: Some(config.into()),
            };
            assert!(find_config_file(project.path(), &cfg).is_none());
        }
    }
