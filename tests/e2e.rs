...
// marks it executable with Unix permissions.
//
// Regression test for a gap where a new file matching an existing
// customManagers pattern wasn't yet tracked in the committed snapshot, so
// `flint run` (no --full) skipped renovate-deps locally and only caught it
// in CI. See src/linters/renovate_deps/manager_patterns.rs.
#[cfg(unix)]
#[test]
fn renovate_deps_local_default_runs_for_new_file_matching_inline_custom_manager_pattern() {
    let repo = git_repo();
...
"#,
    ...
    .unwrap();
    std::fs::write(
        repo.path().join(".github/renovate.json5"),
        r#"{
  customManagers: [
    {
      customType: "regex",
...
"#,
    ...
    assert!(
        repo.path().join(".renovate-ran").exists(),
        "renovate-deps was skipped for a new file matching an inline customManagers pattern; stdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr)
    );
}
...
// marks it executable with Unix permissions.
//
// Same regression as above, but for a repo that only extends flint's own
// preset (`github>grafana/flint`) instead of declaring customManagers
// inline. Flint resolves that preset from its own bundled `default.json`
// offline, without a network call to fetch it.
#[cfg(unix)]
#[test]
fn renovate_deps_local_default_runs_for_new_file_matching_bundled_flint_preset_pattern() {
    let repo = git_repo();
...
"#,
    ...
    std::fs::write(
        ...
        r#"{ extends: ["config:recommended", "github>grafana/flint"] }
"#,
    )
    .unwrap();
    std::fs::write(repo.path().join("renovate-tracked-deps.json"), "{}\n").unwrap();
    ...
    // A brand-new workflow file — matched by default.json's mise-in-workflows
    // custom manager, not by anything declared in this repo's own config.
    std::fs::create_dir_all(repo.path().join(".github/workflows")).unwrap();
    std::fs::write(repo.path().join(".github/workflows/ci.yml"), "name: ci\n").unwrap();
    let out = Command::new("git")
        .args(["add", "-A"])
...
"#,
    ...
    assert!(
        repo.path().join(".renovate-ran").exists(),
        "renovate-deps was skipped for a new file matching flint's bundled preset pattern; stdout:\n{}\nstderr:\n{}",
        String::from_utf8_lossy(&out.stdout),
        String::from_utf8_lossy(&out.stderr)
    );
}
